...
class Planet {
    ...
    move() {
        var r = Math.sqrt(this.r.sizeSquare())
        var v = Math.sqrt(2 * star.mass * data.G / r)
        HTML['vel'].innerText = `vel = ${floor(v, 2)} rad/s`
        HTML['radius'].innerText = `r = ${floor(r / orbit.a, 2)} AU`
        var beforeSeta = this.seta
        //this.seta += v / r
        this.seta += 0.01

        if (Math.floor(beforeSeta / PI2) < Math.floor(this.seta / PI2)) {
            this.T = this.tick
            HTML['T'].innerText = `T = ${this.T}s(3법칙 : ${floor(this.T ** 2 * 1000 / orbit.a ** 3, 2)})`
            this.tick = 0
        }
        this.tick++
    }
